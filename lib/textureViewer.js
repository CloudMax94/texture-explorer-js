...
module.exports = class TextureViewer {
    ...
    generateTexture() {
        ...
        if (textureFormat.hasPalette()) {
            if (this.palette < 1)
                return;
            palette = this.container.file.slice(this.palette, this.palette+0x200);
        }
        this.lock = true;
        var imgBuffer = this.container.file.slice(this.address, this.address+this.width*this.height*textureFormat.sizeModifier());
