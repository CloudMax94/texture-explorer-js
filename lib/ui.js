...
uiHandleProto.createdCallback = function() {
    ...
    var move = (event) => {
        ...
        var direction = window.getComputedStyle(this.parentElement).getPropertyValue('flex-direction');
        if (direction == 'row') { //horizontal
            var width = event.clientX-this.parentElement.offsetLeft;
            prev.style.flexBasis = width+'px';
            next.style.flexBasis = document.body.offsetWidth-width+'px';
        } else { //vertical
            var height = event.clientY-this.parentElement.offsetTop;
