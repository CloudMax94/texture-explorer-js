...
class TextureFormat {
    constructor(data = {}) {
        ...
        if (data.mask) {
            var bitwise = {};
            var bits = 0;
            _.each(data.mask, function(val, key){
                bitwise[key] = {
                    mask: val
                };
            ...
            delete self.data.mask;
            self.data.bitwise = bitwise;
            self.data.bits = bits;
        }
    }
    ...
    pixelsFromData(d) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            var colors = [];
            _.each(self.data.bitwise, function(bitwise){
...
module.exports = {
    ...
    PNGBufferToData: function(buffer) {

    },
    generateTexture: function(imgData, format, width, palette) {
        if (format == textureFormats.jpeg) {
            return null;
        }
        ...
        var imgBuffer = {
            data:   imgData,
            length: imgData.byteLength,
            pos:    0,
            readUIntBE: function(byteLength) {
                var data = 0;
                while (byteLength !== 0) {
                    data = (data<<8)|this.data.getUint8(this.pos);
                    this.pos += 1;
                    byteLength--;
                }
                return data;
    ...
    generateData: function(texture, format) {

        //var ;
        var textureBuffer = {
            //data:   new Buffer(texture.width*texture.height*4),
            data:   new Uint8Array(texture.width*texture.height*format.sizeModifier()),
            pos:    0,
