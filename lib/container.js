...
class TreeItem {
    ...
    getBuffer() {
        return this.workspace.file.slice(this.getData('start'), this.getData('start')+this.getData('width')*this.getData('height')*this.getData('format').sizeModifier());
    }

    getPaletteBuffer() {
        return this.workspace.file.slice(this.parent.getData('start')+this.getData('palette'), this.parent.getData('start')+this.getData('palette')+0x200);
    }
...
class Workspace {
    constructor(workspaces) {
        ...
        this.root = null;
        this.tree = new Tree(this);
        var tab = this.tab = document.createElement('div');
        tab.onclick = (event) => {
            this.workspaces.select(this);
        };
        var tabText = document.createElement('span');
        tabText.classList.add('btnText');
        tabText.innerHTML = 'New Tab';
        var tabClose = document.createElement('span');
        tabClose.classList.add('closeBtn');
        ...
        this.workspaces.element.insertBefore(tab, this.workspaces.newTabElement);
    }
    ...
    setFile(data, info) {
        this.file = data;
        this.info = info;
        this.tab.querySelector('span').innerHTML = info.name;
    }
...
class Workspaces {
    constructor() {
        this.current = null;
        this.workspaces = [];
        this.element = document.getElementById('tabs');
    }
    select(workspace) {
        ...
        if (workspace) {
            workspace.tab.classList.add('selected');
            workspace.tree.show();
            workspace.tree.element.focus();
    ...
    new(select = true) {
        ...
        var preview = new TextureViewer(workspace, document.getElementById('texture-preview'));
        preview.assignInputs(document.getElementById('preview-fields'));
        workspace.tree.attachPreview(preview);
