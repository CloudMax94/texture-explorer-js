...
class Container {
    ...
    initialize(input) {
        var self = this;
        console.time('Initialize profile and generate textures');
        var root = self.root = new TreeItem('directory', this);
        root.createElement();
        root.setData('name',    'Root');
        root.setData('address', 0x00000000);
        root.setData('size',    self.file.byteLength);
        root.updateIcon();
        var types = ['directory', 'texture'];
        function _innerLoop(input, parent) {
            types.forEach(function(type) {
                if (input[itemFormatPlural(type)]) {
                    input[itemFormatPlural(type)].forEach(function(i, index){
                        var item = new TreeItem(type, self);
                        item.createElement();
        ...
        console.log(self);
        console.timeEnd('Initialize profile and generate textures');
    }
